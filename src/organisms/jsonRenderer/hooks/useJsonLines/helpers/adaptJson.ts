...
const adaptPrimitiveField = (fieldName: string, fieldValue: PrimitiveValue): AdaptedJSON => {
...
const adaptFields = (elements: ComplexValue | undefined) => {
...
const adaptJson = (json: ComplexValue | undefined): AdaptedJSON => {
  const dataType = getDataType(json) as ComplexDataType;
  const { size, value } = adaptFields(json);
