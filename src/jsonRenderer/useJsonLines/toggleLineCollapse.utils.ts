...
// Helpers
import { copyLine } from './general.utils';
...
const getPartitionsOnExpandingLine = (lineIndex: number, lines: readonly Line[]) => {
  const partBeforeWithoutLine = lines.slice(0, lineIndex);
  const partAfterWithMatchingClosingSeparator = lines.slice(lineIndex + 1);
...
const getIndexOfClosingSeparator = (lineIndex: number, lines: readonly Line[]) => {
  ...
  const closingSeparatorOffset = remainingLines.findIndex(line => matchClosingSeparatorForDepth(line, depth));
  const indexOfClosingSeparator = closingSeparatorOffset + lineIndex + 1;
...
const getPartitionsOnCollapsingLine = (lineIndex: number, lines: readonly Line[]) => {
  ...
  const partBeforeWithoutLine = lines.slice(0, lineIndex);
  const partAfterWithMatchingClosingSeparator = lines.slice(indexOfClosingSeparator);
